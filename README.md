...
Clone do jogo Angry Birds orientado pelo professor Renzo Nuccitelli durante live de programação na Semana do Programador Profissional (SPP).
...
Explicação detalhada sobre classes e métodos que se encontram nos scripts atores.py e fase.py.
...
Classe que representa porcos na fase e que podem ser destruídos por pássaros. Herda de ator. Seu caracter de representação ativo é a o caracter "@".

### Status

Um porco ao ter seu status alterado para DESTRUIDO deve ter seu caracter de apresentação alterado para "+" (sinal de mais).
...
Cada pássaro deve armazenar esse valor e o tempo de lançamento para cálculo de sua posíção. Lembrar que o tempo das fórmulas é delta_t = T_final - T_inicial.
